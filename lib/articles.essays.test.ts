...
/**
 * The filesystem-backed half of the article registry: reading `content/writing`
 * off disk, and the two formatters that dress an article for a page. The pure
 * transforms underneath (`toIsoDay`, `isEssayFile`, `mergeWriting`) are covered
 * directly in articles.test.ts.
 *
 * The repo ships no essays yet, so rather than commit fixture prose, back the
 * filesystem with an in-memory map and write the essays each test needs. The
...
describe("getEssaySlugs", () => {
  ...
  it("fails the build on a filename that would corrupt the sitemap", () => {
    // Louder here than as an unparseable sitemap in production.
    writeEssay("Speed & Correctness.md", "---\ntitle: x\n---\nbody");
    expect(() => getEssaySlugs()).toThrow(/kebab-case/);
  });
});
...
describe("getEssayContent", () => {
  ...
  it("treats a date that is not an ISO day as undated, and non-list tags as none", () => {
    writeEssay("odd.md", "---\ntitle: O\ndate: 20260102\ntags: notalist\n---\nbody");
