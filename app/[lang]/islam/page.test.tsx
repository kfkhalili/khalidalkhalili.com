...
const renderPage = async (lang = "en") =>
  render(await IslamPage({ params: Promise.resolve({ lang }) }));
...
describe("IslamPage", () => {
  it("heads the page in the reader's language", async () => {
    await renderPage("ar");
    expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent(ar.islam.title);
    expect(screen.getByText(ar.islam.subtitle)).toBeInTheDocument();
  });
  ...
  it("shows a reflection whole and links to the conversation", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: en.islam.reflections }),
    ).toBeInTheDocument();
  ...
  it("names the kind of piece each one is", async () => {
    ...
    expect(screen.getByText(en.islam.reflection)).toBeInTheDocument();
    expect(screen.getByText(en.islam.lesson)).toBeInTheDocument();
  });
  ...
  describe("the quoted ayah", () => {
    ...
    it("labels the quote in the page's own script", async () => {
      getReflections.mockResolvedValue(quoted());
      await renderPage("ar");
      expect(screen.getByText("البقرة 2:48 · Saheeh International")).toBeInTheDocument();
    });
  ...
  describe("the citation chips", () => {
    ...
    it("name the surah in Arabic on the Arabic page", async () => {
      getReflections.mockResolvedValue(feed({ posts: [reflection()] }));
      await renderPage("ar");
      expect(screen.getByRole("link", { name: "البقرة 2:48" })).toBeInTheDocument();
    });
  ...
  describe("the byline", () => {
    ...
    it("leaves an English reflection on the English page unflagged", async () => {
      getReflections.mockResolvedValue(feed({ posts: [reflection()] }));
      await renderPage("en");
  ...
  describe("when there is nothing to show", () => {
    it("says so when QuranReflect answered and I have written nothing", async () => {
      ...
      expect(screen.getByText(en.islam.empty)).toBeInTheDocument();
      expect(
    ...
    it("does not claim an empty shelf when the feed could not be reached", async () => {
      ...
      expect(screen.queryByText(en.islam.empty)).not.toBeInTheDocument();
      expect(
  ...
  describe("generateMetadata", () => {
    it("describes the page at its own locale-qualified URL", async () => {
      const meta = await generateMetadata({ params: Promise.resolve({ lang: "de" }) });
      expect(meta.title).toBe(en.islam.title); // "Islam" in every locale
      expect(meta.alternates?.canonical).toContain("/de/islam");
    });
  });
});
