...
const en: ThirdThingContent = {
  ...
  why: [
    ...
    `It is tempting to hear that sentence as bad management, but it's a product of desperation. The manager cannot fix the code, because it is a black box. Cannot take over operations, because the contract says the vendor operates. Cannot even watch the work happen, because the tickets live in a Jira nobody from the customer can open. Every real lever was signed away years ago. What remains is assigning responsibility and threatening cost transfer. <strong>Blame is what management looks like after every real lever has been contracted away.</strong>`,
...
const de: ThirdThingContent = {
  ...
  why: [
    ...
    `Es ist verlockend, diesen Satz als schlechtes Management zu hören, aber er ist ein Produkt der Verzweiflung. Der Manager kann den Code nicht reparieren, der ist eine Blackbox. Kann den Betrieb nicht übernehmen, der Vertrag sagt, der Lieferant betreibt. Kann der Arbeit nicht mal zusehen, die Tickets liegen in einem Jira, das niemand auf Kundenseite öffnen kann. Jeder echte Hebel wurde vor Jahren per Unterschrift abgegeben. Was bleibt, ist Verantwortung zuweisen und mit Kosten drohen. <strong>Schuldzuweisung ist das, was von Management übrig bleibt, wenn jeder echte Hebel wegverhandelt wurde.</strong>`,
