...
const de: ThirdThingContent = {
  ...
  thirdThing: `Klar sieht das auf dem Papier vernünftig aus. Der Lieferant behält sein geistiges Eigentum, wie eine Produktfirma. Aber er betreibt auch, was er baut, auf deinen Cloud-Konten, wie eine Dienstleistungsfirma. Seine Tickets liegen in seinem Jira, das du nicht sehen kannst. Sein Code ist eine Blackbox. Seine Server treiben deine Rechnung hoch, und nicht mal ihre Größe darfst du bestimmen. Eigentum, Betrieb, Sichtbarkeit: Alle drei zeigen jetzt in verschiedene Richtungen, und keines davon zeigt auf dich.`,
  symptoms: [
  ...
  why: [
    ...
    `Es ist verlockend, diesen Satz als schlechtes Management zu hören, aber er ist ein Produkt der Verzweiflung. Der Manager kann den Code nicht reparieren, der ist eine Blackbox. Kann den Betrieb nicht übernehmen, der Vertrag sagt, der Lieferant betreibt. Kann der Arbeit nicht mal zusehen, die Tickets liegen in einem Jira, auf das nur der Lieferant Zugriff hat. Jeder echte Hebel wurde vor Jahren per Unterschrift abgegeben. Was bleibt, ist Verantwortung zuweisen und mit Kosten drohen. <strong>Schuldzuweisung ist das, was von Management übrig bleibt, wenn jeder echte Hebel wegverhandelt wurde.</strong>`,
    `Beachte, was die Drohung tatsächlich wert ist. Verantwortung dort zu übertragen, wo die Fähigkeiten fehlen, legt nur im Voraus fest, wer am Ende schuld ist. Wer die Server dimensioniert, bezahlt sie nicht, und Unterauslastung ist das Ergebnis. Und wenn das Geld Jahre vor der Arbeit gebunden wurde, ist die Kostendrohung Theater auf Theater: Das Geld ist längst ausgegeben. Jeder im Raum weiß das alles. Der Satz wird trotzdem gesagt. Ihn zu sagen, ist der einzige Hebel, der sich noch ziehen lässt.`,
    `Was kommt bei dem Druck heraus? Nicht die Antwort. Die können sie nicht liefern. Heraus kommt stattdessen Sprache: „Wir schauen uns das nächste Woche an.“ Schuld fließt nach unten, Floskeln fließen nach oben, und beide Seiten gehen mit Papieren nach Hause, die beweisen, dass sie ihren Teil getan haben. Schuld rein, Bullshit raus. Der Dialekt ist nichts weiter als die Art, wie Leute in einem kaputten Modell zurechtkommen.`,
  ],
  simCaption: `Diese Maschine kannst du unten selbst laufen lassen. Solche Projekte nennt man Wassermelonen: außen grün, innen rot. Dreh den Schulddruck hoch und sieh zu, wie die beiden Farben auseinanderdriften.`,
...
const ar: ThirdThingContent = {
  ...
  simCaption: `يمكنك تشغيل هذه الآلة بنفسك هنا. مشاريع كهذه تُسمّى بطيخات: خضراء من الخارج، حمراء من الداخل. ارفع ضغط اللوم وراقب كيف يفترق اللونان.`,
  ...
  audit: `لن تحتاج لتقييم نضوج المورّد. يجب أن يُصاغ العقد بإحدى الطريقتين وبشكلٍ واضح، وللتأكد من صحة العقد أجب عن الأسئلة التالية.`,
  walkTheFloor: `إن لم تصطفّ الإجابات في أحد الشكلين الصريحين، فلا تنتظر الكارثة؛ امشِ في أرض المشروع وستجد الأمراض تتكاثر. جدولٌ يكرّر نفسه. مكالمةٌ يومية يتجاهلها الناس أو يقبلونها على مضض. صفحةٌ لا يجدها أحد. غرفةٌ مليئة بأناسٍ طيبين يتكلمون لغةً لا يمكن فيها لشيءٍ أن يكون خاطئًا.`,
  closing: [
  ...
  sim: {
    ...
    badges: { green: `على المسار`, yellow: `معرَّض للخطر`, red: `حرِج` },
    sprint: `الدورة`,
    running: `● يعمل`,
    paused: `❚❚ متوقّف`,
    log: {
