...
const en: ThirdThingContent = {
  ...
  symptoms: [
    ...
    `But look at the wreckage. Nothing was hidden, and yet nobody can find the current truth anymore. Information has multiplied until nobody knows which copy to trust. <strong>The truth does not get hidden. It gets outnumbered.</strong>`,
  ],
  ...
  closing: [
    ...
    `And when the chance comes, remember that the split costs nobody their job. It is the same people, sorted into shapes where they are measured on what they are actually good at. The scientists get to be scientists. The manager gets real levers instead of blame. Nobody maintains the spreadsheet, nobody schedules the daily, nobody answers in hedges. That is the most hopeful fact in all of this: nobody in these rooms wants the bullshit. Not the veteran, not the manager, not the vendor. So ask the four questions, out loud, even if you are the newest person in the room. A question is not an accusation. People return to plain sentences the moment plain ones become affordable again. <strong>It's not the people you need to fix, it's the model.</strong>`,
  ],
  statusPage: {
...
const de: ThirdThingContent = {
  ...
  symptoms: [
    ...
    `Aber sieh dir die Trümmer an. Nichts wurde versteckt, und trotzdem findet niemand mehr die aktuelle Wahrheit. Information hat sich vervielfältigt, bis niemand mehr weiß, welcher Kopie er trauen soll. <strong>Die Wahrheit wird nicht versteckt. Sie gerät in die Unterzahl.</strong>`,
  ],
  ...
  why: [
    ...
    `Was presst dieser Druck aus dem Lieferanten heraus? Nicht die Antwort; die können sie nicht liefern. Er presst Sprache heraus: „Wir schauen uns das nächste Woche an." Schuld fließt nach unten, Floskeln fließen nach oben, und beide Seiten gehen mit einer Papierspur nach Hause, die beweist, dass sie ihren Teil getan haben. Schuld rein, Bullshit raus. Der Dialekt ist, wie Menschen in einem kaputten Modell atmen.`,
  ],
...
const ar: ThirdThingContent = {
  ...
  why: [
    ...
    `ماذا ينتزع ذلك الضغط من المورّد؟ ليس الجواب؛ فهم عاجزون عن إنتاجه. إنه ينتزع لغةً: «سننظر في الأمر الأسبوع المقبل». اللوم يتدفق نزولًا، والتحوّطات تتدفق صعودًا، ويغادر الطرفان وبيد كلٍّ منهما أثرٌ ورقي يثبت أنه أدّى دوره. لومٌ يدخل، هراءٌ يخرج. اللهجة هي طريقة الناس في التنفّس داخل نموذجٍ مكسور.`,
  ],
  ...
  closing: [
    ...
    `وحين تأتي الفرصة، تذكّر أن الفصل لا يكلّف أحدًا وظيفته. إنهم الأشخاص أنفسهم، وقد فُرزوا في أشكالٍ يُقيَّمون فيها على ما يجيدونه فعلًا. العلماء يعودون علماء. والمدير ينال روافع حقيقيةً بدلًا من اللوم. لا أحد يصون الجدول، ولا أحد يجدول الاجتماع اليومي، ولا أحد يجيب بالتحوّطات. تلك أرجى حقيقةٍ في الأمر كله: لا أحد في هذه الغرف يريد الهراء. لا المخضرم، ولا المدير، ولا المورّد. فاطرح الأسئلة الأربعة بصوتٍ مسموع، حتى لو كنت أحدثَ من في الغرفة. السؤال ليس اتهامًا. يعود الناس إلى الجمل الصريحة لحظةَ تصبح الجمل الصريحة في المتناول من جديد. <strong>ليس الناس هم من يحتاج إلى إصلاح، بل النموذج.</strong>`,
  ],
  statusPage: {
