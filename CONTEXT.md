...
Domain language for **khalidalkhalili.com**: a personal site of interactive
explorable explanations, essays, and notes.
...
- **Explorable**: an interactive explanation whose body is a React component (e.g.
  ...
  labels) lives in a co-located content module.
...
- **Tag**: what a piece is about, shown on its card and in its header. A tag is a place rather than a label: it links
...
- **Sim**: the interactive widget an Explorable is built around, and the thing the
  ...
  **Sim model** that is pure and knows nothing of copy, and a view that renders it.
...
- **Layered copy** (`Layered` in `components/tech-debt-sim.tsx`): a block of Sim
  ...
  similar length, so the reserve is the copy rather than padding.
...
- **Reflection**: something I wrote about an ayah, published on QuranReflect and
  ...
  Quran.com, and it dates itself in the language I wrote it in: the site
  publishes in English, but a Reflection can be written in Arabic or German, and
  its date, its body's direction, and the rendering under its quoted ayat all
  follow the Reflection rather than the page (the language badge is what tells
  the reader they differ). That language lives in `lib/quran-reflect.ts` as
  `ReflectionLang`: it is about the feed's content, not the site's chrome.
- **Page copy**: the chrome-adjacent prose (home hero, about) authored in
  `content/*.md`.
- **Chrome copy** (`lib/strings.ts`): every nav label, button, and heading the
  site's furniture carries, in one module. Page prose lives in `content/`;
  article copy lives with the article.
- **Share link**: a page's absolute URL. For an article the same string is what
  a reader copies, what `og:url` claims, what the canonical tag points at, and
  what the sitemap lists.
- **Byline**: an Article's date and reading time.
- **Share card**: the 1200×630 image a shared link unfurls into, drawn per
  article from the article's own metadata.
...
- **Sim model** (`*.model.ts` beside each sim): the pure model a Sim draws,
  ...
  React, CSS, or copy. It answers in domain terms: a step returns the next
...
- **Content document** (`lib/content.ts`): the one module that turns a markdown
  ...
  read-plus-frontmatter primitive; `readContent` adds the render.
  Both page copy and essays cross this interface; nothing else reads markdown.
- **Article render seam** (`app/writing/[slug]`): the one place an article
  renders. Two adapters satisfy it: a **markdown adapter** (essays → HTML) and a
  **component adapter** (explorables → their `Body`).
...
- **Share link** (`lib/share.ts`): the one module that knows where a page lives
  ...
  tag, its `og:url`, the reader-facing copy button,
...
- **Tag link** (`lib/tags.ts`): the one module that knows a tag is a place. It
  ...
  one way and read another; escaping happens once, which is what keeps a space
  or an ampersand from rewriting the query. Type-only import of
...
- **Page self-description** (`lib/page-metadata.ts`): the one place a page says
  ...
  is that block, built from the page's own path. The layout therefore
...
- **Site inventory** (`app/sitemap.ts`): the one statement of which pages the
  site offers for indexing, and when each was published. It
  reads the same registries the pages render from, so a new article
...
- **Share card** (`app/writing/[slug]/opengraph-image`): the article's
  metadata rendered as an image. It carries the site's own constraints: satori
  can't see `next/font`, so the faces are vendored in `assets/fonts/`, the
  Arabic face included, so an Arabic term quoted in a title never renders as
  tofu.
